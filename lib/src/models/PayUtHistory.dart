...
class PayUtHistory {
  ...
  PayUtHistory.fromJson(dynamic json) {
    credit = json['credit'];
    if (json['historique'] != null) {
      historique = [];
      json['historique'].forEach((v) {
        historique?.add(PayUtItem.fromJson(v));
      });
    }
  }

  num? credit;
  List<PayUtItem>? historique;
}
...
class PayUtItem {
  PayUtItem.fromJson(dynamic json) {
    amount = json['amount'];
  ...
  String nameExtracted(BuildContext context) {
    if (isVirement)
      return "${Translate.of(context).transfertPayut}${isOutAmount ? "(${Translate.of(context).sendedTransfertPayut})" : "(${Translate.of(context).reveivedTransfertPayut})"}";
    if (type == "RECHARGE") return Translate.of(context).reloading;
    return "${quantity!.abs().toInt()} ${name}";
  }

  bool isAtSameDay(PayUtItem other) {
